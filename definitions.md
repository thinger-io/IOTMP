...
IOTMP depends highly on JSON format for encoding information payloads. JSON fields can hold numbers, strings, object or arrays. Moreover, some JSON alternatives like PSON, MessagePack, CBOR, BSON, etc., supports also encoding binary information as a value. In IOTMP, an `any` field represents any value encoded that can be encoded with such formats.
